        ...
        try {
            ...
            for (const call of calls) {
                ...
                await socket.sendMessage(from, {
                    text: userConfig.REJECT_MSG || '*CALL REJECTED ☺️*'
                });
                console.log(`CALL REJECT NO CALLS ALLOWED${number} from ${from}`);
            }
        } catch (err) {
            ...
            try {
                ...
                if (["send", "sendme", "sand"].includes(cmdNoPrefix)) {
                    if (!mek.message.extendedTextMessage?.contextInfo?.quotedMessage) {
                        await conn.sendMessage(from, { text: "*Reply to a status update 😊*" }, { quoted: mek });
                    } else {
                        try {
